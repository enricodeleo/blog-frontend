...
I found exactly that race in `bullmq-rs` v2.2.0, the community Rust port of BullMQ. It is reproduced in [PR #21](https://github.com/bogardt/bullmq-rs/pull/21), which proposes the repair. At the time of writing, that PR is open: this is a defect in the released crate, not a victory lap about a fix that has shipped.
...
BullMQ has been my default queue for about five years. I have used it to keep request paths small: work that does not need to determine the HTTP response goes to the queue. When I wanted Rust workers on those queues, I joined `bullmq-rs` and contributed a meaningful part of its code.

Then BullMQ shipped [`bullmq-official`](https://bullmq.io/news/260712/rust-release/), its official Rust client. This is not a neutral buyer's guide: I have helped build one side of the comparison. But the conclusion is still straightforward.

**For a new system where BullMQ compatibility is the contract, use the official client.** Not because its Rust orchestration has had more time in production — it has not — but because its authors own the state-transition protocol. For a Rust-only system, `bullmq-rs` still has the better type ergonomics. Those are different kinds of confidence.

The race, the type trade-off, and a benchmark of both clients are enough to explain why.

## A double-execution race in the worker loop

This is the part worth leading with, because it's the kind of bug that never shows up in a README or a feature checklist.
...
The proposed repair mirrors the official ordering: reserve capacity first, then claim. This is the single most technically important difference I found.

## The one thing bullmq-rs got right: typed jobs

For all of that, `bullmq-rs` has one genuinely better idea.
...
Despite all that, `bullmq-official` has one overwhelming advantage that has little to do with API taste: **it owns the protocol.**
...
That gap is tiny when maintainers are active. Over five years, it becomes the maintenance budget. This is the strongest argument for the official client, and it barely touches code quality.
...
**What I would do with `bullmq-rs` from here** — beyond getting the race repair merged and released: keep pursuing parity, but treat it as a compatibility contract rather than a feature race it can win. Version-map releases to upstream BullMQ, port every script and API change, and make bidirectional Node↔Rust checks the evidence behind every compatibility claim. Upstream owns the protocol; the port must make drift visible and expensive.

The opportunity is to meet that contract through the best Rust experience of it: typed input *and* typed output (`Worker<TInput, TOutput>`), ergonomic enums, structured errors, and Tokio-native cancellation. Be the strongly typed, Rust-opinionated client for BullMQ-compatible queues. Parity gets users through the door; a better Rust API gives them a reason to stay.
...
| Worker fetch architecture | Concurrency slot before fetch; N loops | Central loop; claim before capacity in v2.2.0; [repair proposed](https://github.com/bogardt/bullmq-rs/pull/21) | **Official today** |
...
The comparison covers `bullmq-official` 1.1.0, released `bullmq-rs` 2.2.0, and the open PR #21 branch. On an M4, the median of five runs says: the official client leads sequential adds (≈18.3k vs ≈13–15k jobs/s); `bullmq-rs` wins parallel adds (≈118k vs ≈95k jobs/s); and under a steady producer both sit at sub-millisecond p50 latency. These are localhost measurements of client-side overhead, not production capacity figures.
...
The other dramatic gap — worker throughput on a pre-filled queue — is not slow processing but a single 5-second `BZPOPMIN` cold-start stall in the `bullmq-rs` fetch loop. After it wakes, it drains at a rate comparable to the official client. PR #21 avoided that stall in three of five runs at concurrency 100, but not in the concurrency-10 scenario, so it is not a repair for the underlying cold-start behavior. And Redis commands per job came out identical everywhere, which is worth sitting with: the official client's edge is not wire efficiency, it is orchestration. The parallel-add win shows the trade still cuts both ways.

The useful conclusion is not that one client won a feature race. It is that a queue does not care whether its client has a pleasant API when it decides a lost job is safe to rerun. It cares about the state machine. For a shared BullMQ system, I would rather not own a fork of that state machine.
