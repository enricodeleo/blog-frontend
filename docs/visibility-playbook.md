...
- **English-first essays** — written directly in English, no Italian original, no `translation` field. Reserved for pieces where the underlying event is itself international: an open-source contribution, a bug found and reported upstream, a released library. `narrowed-not-superseded` (published as "BullMQ in Rust: what the official client leaves me to build") is the first of these — it exists because the event it describes (an outside contributor's Rust port of BullMQ, benchmarked head-to-head against the newly-released official client and found faster on parallel adds) has no Italian audience to begin with.
